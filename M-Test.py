import os
import webbrowser
webbrowser.open('https://t.me/TEAM_CLOWNS_66')
...
try:
    ...
    (X + ' \n\nTOKEN')
    print('\n')
    tok = input(X + '  Inter Your Token ===>> ' + C)
    sleep(0.1)
    (A + ' ID')
    print('\n')
    ID = input(Y + ' Inter Your Tele Id ===>> ' + C)
    sleep(0.1)
    start_msg = requests.post(f"https://api.telegram.org/bot{tok}/sendMessage?chat_id={ID}&text=  جاراً الصيد انتظر الاداة برعاية Clowns Team الاداة من صنع Clown Num-1 للتواصل @T_C66bot ").json()
